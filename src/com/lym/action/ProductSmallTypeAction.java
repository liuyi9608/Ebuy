...
public class ProductSmallTypeAction extends ActionSupport {
	...
	public String list()throws Exception{
		...
		long total=productSmallTypeService.getCountSmallType(s_productSmallType);
		JsonConfig jsonConfig=new JsonConfig();
		jsonConfig.setExcludes(new String[]{"productList"});
		jsonConfig.registerJsonValueProcessor(ProductBigType.class,new ObjectJsonValueProcessor(new String[]{"id","name"}, 
	...
	public String save() throws Exception{
		productSmallTypeService.saveSmallType(productSmallType);
		JSONObject result=new JSONObject();
	...
	public String delete()throws Exception{
		...
		for(int i=0;i<idsStr.length;i++){
			if(productService.existProductWithSmallType(Integer.parseInt(idsStr[i]))){
				result.put("exist", "商品小类包含商品");
			}else{
				ProductSmallType productBigType=productSmallTypeService.getProductSmallTypeById(Integer.parseInt(idsStr[i]));
				productSmallTypeService.deleteSmallType(productBigType);			
			}
		}
		result.put("success", true);
		ResponseUtil.write(ServletActionContext.getResponse(), result);
