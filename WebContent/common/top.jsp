	...
	<div class="help">
		<c:choose>
			<c:when test="${not empty currentUser }">
				<a href="user_userCenter.action">用户:${currentUser.userName }</a>
				<a href="user_userCenter.action">个人中心</a>
	...
	<div class="navbar">
		<ul class="clearfix">
			...
			<c:forEach var="bigType" items="${bigTypeList }">
				<li><a href="product.action?s_product.bigType.id=${bigType.id}">${bigType.name}
				</a></li>
			</c:forEach>
		</ul>
	</div>
				...
				<c:forEach var="tag" items="${tagList }" varStatus="status">
					<c:choose>
						<c:when test="${status.index==0 }">
							<li class="first"><a href="#" target="_blank">
									${tag.name } </a></li>
						</c:when>
						<c:otherwise>
							<li><a href="#" target="_blank"> ${tag.name } </a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			</ul>
